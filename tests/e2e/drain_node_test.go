			...
			It("can drain the primary node and recover the cluster when uncordoned", func() {
				...
				By("verifying the primary is now pending or somewhere else", func() {
					Eventually(func(g Gomega) {
						...
						g.Expect(err).ToNot(HaveOccurred())
						GinkgoWriter.Println(fmt.Sprintf("Drained node: %v", drainedNodeName))
						GinkgoWriter.Println(fmt.Sprintf("Pod %v is on node %v", pod.Name, pod.Spec.NodeName))
						GinkgoWriter.Println(fmt.Sprintf("Pod status: %v", pod.Status.Phase))
