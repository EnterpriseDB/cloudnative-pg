...
func (r *ClusterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, maxConcurrentReconciles int) error {
	...
	ctrlBuilder := ctrl.NewControllerManagedBy(mgr).
		...
		Watches(
			...
			// Database/Publication/Subscription watches below cover: the initial
			// cache sync on restart (Create is unfiltered by GenerationChanged) and
			// a role picking up its own deletionTimestamp later while the operator
			// keeps running, which does not by itself bump the generation.
